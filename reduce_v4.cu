...
// 调换相加的顺序
// 为后续使用共享内存时，减少 bank conflict
__global__ void reduce_sum_v4(float *input, float *output, int n) {
    // 在每个 block 内进行独立的 reduce
    int size = blockDim.x * 2;
    int offset = blockIdx.x * size;
    int tid = threadIdx.x;
    for (int stride = size / 2; stride > 0; stride >>= 1) { // stride /= 2
        // 每个线程负责一次 reduce
        if (tid < stride && offset + tid + stride < n) {
...
int main() {
    ...
    reduce_sum_v4<<<numBlocks, blockSize>>>(input_d, output_d, n);
